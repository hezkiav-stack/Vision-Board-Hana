...
const KingdomDoor: React.FC<KingdomDoorProps> = ({ isOpen, onOpen }) => {
  ...
  const handleOpen = () => {
    playSound(SOUNDS.DOOR_OPEN, 0.4);
    onOpen();
  };
  ...
  return (
    <div className={`fixed inset-0 z-50 flex overflow-hidden transition-transform duration-1000 ${isOpen ? 'pointer-events-none' : ''}`}>
      ...
      <div 
        onClick={handleOpen}
        className={`relative w-1/2 h-full bg-[#8b4513] border-r-4 border-yellow-500 transition-transform duration-1000 ease-in-out cursor-pointer origin-left shadow-2xl flex items-center justify-end
      ...
      >
        ...
        <div className="mr-8 relative z-10">
           <div className="w-16 h-16 rounded-full bg-yellow-500 border-4 border-yellow-200 shadow-lg flex items-center justify-center animate-handle-glow">
      ...
      <div 
        onClick={handleOpen}
        className={`relative w-1/2 h-full bg-[#8b4513] border-l-4 border-yellow-500 transition-transform duration-1000 ease-in-out cursor-pointer origin-right shadow-2xl flex items-center justify-start
          ${isOpen ? 'translate-x-full -rotate-y-90' : 'translate-x-0'}
        `}
        style={{
          backgroundImage: 'radial-gradient(circle at center, #a0522d 0%, #8b4513 100%)',
      ...
      {!isOpen && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-white/95 px-12 py-8 rounded-[3rem] shadow-2xl border-4 border-yellow-400 animate-pulse transform -rotate-2 flex flex-col items-center">
            <h1 className="text-4xl md:text-6xl font-pacifico text-pink-500 text-center drop-shadow-sm">
              Kingdom of Vision
            </h1>
            <p className="text-center text-black mt-4 font-bold text-lg md:text-xl">Klik untuk Masuk ✨</p>
            <p className="text-center text-black mt-2 text-xs md:text-sm font-semibold italic">
              nyalakan volume untuk pengalaman yang lebih seru 🔊
            </p>
          </div>
        </div>
      )}
